        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (downInCircle && inCircle) {//down按在渐变色环内, 且move也在渐变色环内
                    ...
                    VDLog.v(TAG, "色环内, 坐标: " + x + "," + y);
                } else if (downInRect && inRect) {//down在渐变方块内, 且move也在渐变方块内
                    mCenterPaint.setColor(interpRectColor(mRectColors, x));
                }
                VDLog.v(TAG, "[MOVE] 高亮: " + highlightCenter + "微亮: " + highlightCenterLittle + " 中心: " + inCenter);
                ...
                if (mColorChangeListener != null) {
                    mColorChangeListener.colorChanged(red,green,blue);
                }
                invalidate();
                break;
            case MotionEvent.ACTION_UP:
                if (highlightCenter && inCenter) {//点击在中心圆, 且当前启动在中心圆
                    if (mColorChangeListener != null) {
                        mColorChangeListener.colorChanged(Color.rgb(red, green, blue));
//                        mColorChangeListener.colorChanged(red,green,blue);
                    }
                }
                if (downInCircle) {
...
//	}
    ...
    public void setmColorChangeListener(OnColorChangedListener mColorChangeListener) {
        this.mColorChangeListener = mColorChangeListener;
    }

    public void changCenterCircleColor(int color) {
