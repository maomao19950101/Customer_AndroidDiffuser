...
public class Main2Fragment extends BaseFragment implements OnClickListener {
...
//       	  productIV.setImageDrawable(getResources().getDrawable(R.drawable.bg_pot));
       	  productIV.setImageDrawable(getResources().getDrawable(R.drawable.pt_z02));
      	productTypeIconIV.setImageDrawable(getResources().getDrawable(R.drawable.pz_z02));
		...
		mistTB.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					...
					// Light动画
					/*//modify 2017/3/23  隐藏动画 不展示 START
					 * 
		...
		lightTB.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					...
					END
					*/
					handler.sendEmptyMessageDelayed(4, 10);//add 2017-03-27
				} else {
					lightView.hide();
...
//					add 2017-03-27
		...
		motorTB.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton compoundButton, boolean isChecked) {
				...
				//	switchFlowers(true);	modify 2017/3/23  隐藏动画 不展示 START
				} else {
					oilsView.hide();
					//switchFlowers(false);	modify 2017/3/23  隐藏动画 不展示 START
				}
			}
		});
    ...
    private Handler handler = new Handler(){
    	public void handleMessage(android.os.Message msg) {
    		...
    		if(what==1){//PERF
    			...
    			bleManager.writeData(CHexConver.hexStr2Bytes( AppConstant.HardwareCommands.CMD_RGB_ON), false);
    		}else if(what==5){
    		...
    		}else if(what==9){//发送整个设备 开
    			switchTB.setChecked(false);
    		}else if(what==10){
    			mistTB.setChecked(false);
    		}else if(what==11){
    			soundTB.setChecked(false);
    		}else if(what==12){
    			lightTB.setChecked(false);
    			//关闭RGB
    			bleManager.writeData(CHexConver.hexStr2Bytes( AppConstant.HardwareCommands.CMD_RGB_OFF), false);
    		}
    	};
		...
		public void responseOrNotify(int[] data) {
			...
			if (l > 2) {
				...
				case 0x04://mist
					...
					break;
				case 0x0c://music
					/*if(data[4]==1)handler.sendEmptyMessageDelayed(5, 10);
					else if(data[4]==0)handler.sendEmptyMessageDelayed(11, 10);*/
					if(data[4]==1&&mstatus)handler.sendEmptyMessageDelayed(5, 10);
					mstatus=false;
					break;
				case 0x0e://Light
if(data[4]==1)handler.sendEmptyMessageDelayed(4, 10);
else if(data[4]==0)handler.sendEmptyMessageDelayed(12, 10);
lstatus=false;
					break;
				case 0x01://switch
					if(data[4]==1)handler.sendEmptyMessageDelayed(8, 10);
					else if(data[4]==0)handler.sendEmptyMessageDelayed(9, 10);
					sstatus=false;
				break;
				default:
					break;
